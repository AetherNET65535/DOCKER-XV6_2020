...
e1000_transmit(struct mbuf *m)
{
  ...
  // if this block is using by network card
...
e1000_recv(void)
{
  ...
  // while this block received data
  while(rx_ring[i].status & E1000_RXD_STAT_DD){
    rx_mbufs[i]->len = rx_ring[i].length;
    struct mbuf *rb = rx_mbufs[i];

    // &rb != &rx_mbufs[i], addr not same
    // former is the received data block
    // latter is the new data block
    rx_mbufs[i] = mbufalloc(0);
    ...
    // let net_rx() send it to the process which requesting
    net_rx(rb);
